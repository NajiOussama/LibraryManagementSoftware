...
public class EmprunterController {
    ...
    void emprunter(ActionEvent event) throws SQLException {
        ...
        if (mstudnumber.isEmpty() || mpass.isEmpty() || mtit.isEmpty() || misbn.isEmpty()) {
            ...
            String verifLivre = "SELECT * FROM Livre WHERE Titre =? and ISBN = ?";
            PreparedStatement authenticateStatement = connection.prepareStatement(verifLivre);
            authenticateStatement.setString(1,mtit );
            authenticateStatement.setString(2, misbn);
            ResultSet res2 = authenticateStatement.executeQuery();
            if (res.next() ) {
                if (res2.next()) {
                    try {
                    ...
                    alert.setTitle("Message d'erreur");
                    alert.setHeaderText(null);
                    alert.setContentText("Titre ou ISBN incorrect.");
                    alert.showAndWait();
